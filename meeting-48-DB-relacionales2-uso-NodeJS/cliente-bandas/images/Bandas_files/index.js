...
async function editBandFunction(button) {
	...
	try {
		...
		document.getElementById("startDateBand").value = dateStart;
		const yearFinish = info.fecha_separacion.substr(0, 4);
		const monthFinish = info.fecha_separacion.substr(5, 2);
		const dayFinish = info.fecha_separacion.substr(8, 2);
		const dateFinish = `${yearFinish}-${monthFinish}-${dayFinish}`;
		document.getElementById("endDateBand").value = dateFinish;
